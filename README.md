...
> 不是一个带聊天窗口的 CRUD 系统，而是一个 **AI 原生的业务决策平台**。
> AI 不是附属功能，而是贯穿购票、改签、候补、出行规划、客服、运维全链路的 **核心业务智能层**。

---

## 项目定位

传统 12306 类系统的核心是 **高并发 + 状态机**，AI 仅用于客服问答。RailMind AI 重新定义了 AI 在票务系统中的角色：
...
|------|------|------|
| Day 1 | 项目骨架 + 公共模块 | ✅ 完成 |
| Day 2 | Docker 环境 + 数据库初始化 | ✅ 完成 |
| Day 3 | 用户服务 (注册/登录/JWT) | ✅ 完成 |
| Day 4 | 车次服务 (站点/车次/票价) | ✅ 完成 |
| Day 5 | 票务服务 (余票查询 + 库存管理) | ✅ 完成 |
| Day 6 | 票务服务 (锁座 + 候补) | ✅ 完成 |
| Day 7 | 订单服务 (下单 + 状态机) | ✅ 完成 |
| Day 8 | 订单服务 (退票 + 改签) | 🔲 待开发 |
| Day 9 | 支付服务 | 🔲 待开发 |
| Day 10 | 消息服务 (WebSocket) | 🔲 待开发 |
| Day 11-13 | AI 服务 (6 个 Agent) | 🔲 待开发 |
| Day 14-16 | 网关 + 限流 + 压测 | 🔲 待开发 |
| Day 17-18 | Vue3 前端 | 🔲 待开发 |
| Day 19-20 | Docker 部署 + 文档 | 🔲 待开发 |
...
**问题**: 如何让 AI 不只是聊天机器人，而是真正的业务决策层？

**方案**:
- AgentRouter 意图识别 + 路由，支持混合意图串行/并行执行
- Function Calling 工具层，AI 可调用业务接口
- 多轮对话记忆 (Redis 短期 + MySQL 用户画像 + 向量库长期)
- RAG 知识库，政策文档向量化检索

### 2. 高并发抢票方案

**问题**: 如何保证 5000 QPS 下单不超卖？

**方案**:
```
Redis Lua 原子预扣 → Kafka 削峰 → MySQL 落库 → 乐观锁兜底
```
- 第一道防线: Redis Lua 原子操作，检查+扣减一步完成
- 第二道防线: Kafka 异步下单，削峰填谷
- 第三道防线: MySQL 乐观锁，最终一致性保证

### 3. 三级缓存架构

**问题**: 余票查询 QPS 50,000+ 如何实现？

**方案**:
```
Caffeine (5s, L1) → Redis (30s, L2) → MySQL (L3)
```
- 热门线路命中率 > 80% 在 L1 解决
- L2 Redis 承接大部分请求
- L3 MySQL 只处理缓存穿透
...
**方案**:
- Kafka 9 个 Topic，事件驱动
- Outbox 本地消息表，保证至少一次投递
- 幂等消费，消息 ID 去重
...
**方案**:
```
Redisson 分布式锁 (train:date:seat) + MySQL 乐观锁 + 15min TTL 自动释放
```
...
> **RailMind AI - AI 驱动的智能铁路售票平台**
>
> 基于 Spring Boot 3 + Spring AI 的企业级铁路售票系统，集成 6 个 AI Agent 实现智能购票、改签分析、候补预测、出行规划、RAG 客服和运维分析。
>
> **核心职责**:
> - 设计 AI Agent 架构: AgentRouter 意图识别 + 12 个 Function Calling 工具 + RAG 知识库，AI 贯穿购票全链路而非附属聊天窗口
> - 实现高并发抢票方案: Redis Lua 原子预扣 + Kafka 削峰异步下单 + 乐观锁防超卖，支撑 QPS 5000+
> - 构建三级缓存架构: Caffeine → Redis → MySQL，余票查询 QPS 50000+，P99 ≤ 30ms
> - 设计事件驱动架构: Kafka 9 个 Topic 异步解耦 + Outbox 本地消息表保证最终一致性
> - 实现 RAG 智能客服: 知识库向量化 + 混合检索 + 流式回答 + 多轮对话记忆
