...
> ORM 框架：MyBatis Plus（替代 Spring Data JPA）
...
### 实体类规范
- 使用 `@TableName("t_xxx")` 注解指定表名
- 使用 `@TableId(type = IdType.AUTO)` 注解主键
- 使用 `@TableField("column_name")` 注解字段映射
- 使用 `@TableLogic` 注解逻辑删除字段
- 使用 `@Version` 注解乐观锁字段

### Mapper 接口规范
- 继承 `BaseMapper<Entity>` 接口
...
- 使用 `@Transactional` 注解事务管理

### XML 文件规范
- 放置在 `src/main/resources/mapper/` 目录
- 文件名与 Mapper 接口一致
- 复杂关联查询、动态 SQL 在 XML 中编写
...
| MyBatis Plus 配置 | 各模块 Mapper 扫描 + 分页插件 | `@MapperScan` + 分页插件配置 + 逻辑删除配置 |
| application.yml 配置 | 各模块 `application.yml` | MyBatis Plus 配置（mapper-locations / type-aliases-package） |
...
| 任务 | 文件 | 说明 |
|------|------|------|
| 用户 Entity | `domain/model/User.java` | MyBatis Plus Entity (`@TableName`) |
| 乘车人 Entity | `domain/model/Passenger.java` | MyBatis Plus Entity (`@TableName`) |
| User Mapper | `mapper/UserMapper.java` + `xml/UserMapper.xml` | BaseMapper + 自定义查询 |
| Passenger Mapper | `mapper/PassengerMapper.java` + `xml/PassengerMapper.xml` | BaseMapper + findByUserId |
...
- MyBatis Plus: Entity + Mapper + Service接口+实现
- Swagger API文档
```
...
| 任务 | 文件 | 说明 |
|------|------|------|
| Station Entity + Mapper | `domain/model/Station.java` + `mapper/StationMapper.java` | 站点实体 + 数据访问 |
| Train Entity + Mapper | `domain/model/Train.java` + `mapper/TrainMapper.java` | 列车实体 + 数据访问 |
| TrainStation Entity + Mapper | `domain/model/TrainStation.java` + `mapper/TrainStationMapper.java` | 途经站实体 + 数据访问 |
| SeatType Entity + Mapper | `domain/model/SeatType.java` + `mapper/SeatTypeMapper.java` | 座位类型实体 + 数据访问 |
| TrainSchedule Entity + Mapper | `domain/model/TrainSchedule.java` + `mapper/TrainScheduleMapper.java` | 运行计划实体 + 数据访问 |
...
- 票价引擎: 里程×座位系数计算
- 运行计划: 按模板生成每日运行图
- MyBatis Plus: 5个Entity + 5个Mapper + XML复杂查询
- 初始数据: 500站点 + 50列热门车次
```
...
| InventoryMapper | `mapper/InventoryMapper.java` + `xml/InventoryMapper.xml` | 库存数据访问 + 自定义SQL |
...
- 分段限售: 区间票额分配算法
- 批量查询: 同日多车次并行查询
- MyBatis Plus: InventoryMapper + 自定义SQL查询
```
...
| 任务 | 文件 | 说明 |
|------|------|------|
| SeatLock Entity + Mapper | `domain/model/SeatLock.java` + `mapper/SeatLockMapper.java` | 锁座记录 + 数据访问 |
| Waitlist Entity + Mapper | `domain/model/Waitlist.java` + `mapper/WaitlistMapper.java` | 候补记录 + 数据访问 |
...
- 候补统计: 基于历史退票率计算成功率
- MyBatis Plus: SeatLockMapper + WaitlistMapper
```
...
| 任务 | 文件 | 说明 |
|------|------|------|
| Order Entity + Mapper | `domain/model/Order.java` + `mapper/OrderMapper.java` | 订单实体 + 数据访问 |
| OrderItem Entity + Mapper | `domain/model/OrderItem.java` + `mapper/OrderItemMapper.java` | 订单明细实体 + 数据访问 |
| Outbox Entity + Mapper | `domain/model/Outbox.java` + `mapper/OutboxMapper.java` | 本地消息表 + 数据访问 |
...
- 幂等设计: Token机制 + 唯一约束
- 本地消息表: 保证Kafka至少一次投递
- MyBatis Plus: OrderMapper + OrderItemMapper + OutboxMapper
```
...
| 任务 | 文件 | 说明 |
|------|------|------|
| Payment Entity + Mapper | `domain/model/Payment.java` + `mapper/PaymentMapper.java` | 支付实体 + 数据访问 |
...
- 退款: 退票触发退款, 原路退回
- 对账: 定时校验订单-支付金额一致性
- MyBatis Plus: PaymentMapper + 自定义查询
```
...
| Conversation Entity + Mapper | `domain/model/AiConversation.java` + `mapper/AiConversationMapper.java` | 会话实体 + 数据访问 |
| AiMessage Entity + Mapper | `domain/model/AiMessage.java` + `mapper/AiMessageMapper.java` | 消息实体 + 数据访问 |
...
- 知识库: 退改签政策/旅客规程/常见问题
- 会话管理: Redis存储短期记忆
- MyBatis Plus: AiConversationMapper + AiMessageMapper
```
...
| AiLog Entity + Mapper | `domain/model/AiLog.java` + `mapper/AiLogMapper.java` | AI调用日志 + 数据访问 |
...
- MyBatis Plus: AiLogMapper + 自定义统计查询
```
...
### 与 Spring Data JPA 的主要区别

| 对比项 | Spring Data JPA | MyBatis Plus |
|--------|----------------|--------------|
| 实体注解 | `@Entity`, `@Table`, `@Id` | `@TableName`, `@TableId`, `@TableField` |
| 数据访问 | `Repository` 接口 | `Mapper` 接口 + XML |
| 查询方式 | 方法名约定 (`findByXxx`) | `QueryWrapper` / LambdaQueryWrapper / XML |
| 关联查询 | `@OneToMany`, `@ManyToOne` | XML 手动编写 JOIN |
| 分页 | `Pageable` + `Page` | `IPage` + 分页插件 |
| 事务 | `@Transactional` | `@Transactional`（相同） |
| 逻辑删除 | `@SQLDelete` | `@TableLogic` |
| 乐观锁 | 手动实现 | `@Version` 注解 |

### MyBatis Plus 优势

1. **SQL 可控**：复杂查询在 XML 中编写，性能可优化
2. **学习成本低**：SQL 为主，符合直觉
3. **灵活性高**：支持动态 SQL、多表关联、存储过程
4. **性能更好**：无需 ORM 映射开销
5. **国内主流**：国内企业使用率 > 70%
...
| 场景 | JPA | MyBatis Plus |
|------|-----|--------------|
| 简单 CRUD | ⭐⭐⭐⭐⭐ 自动生成 | ⭐⭐⭐⭐⭐ BaseMapper 同样自动 |
| 复杂查询 | ⭐⭐ 需要 @Query 或 Specification | ⭐⭐⭐⭐⭐ XML 灵活编写 |
| 多表关联 | ⭐⭐⭐ Fetch Join | ⭐⭐⭐⭐⭐ XML JOIN |
| 性能调优 | ⭐⭐ 难以优化 SQL | ⭐⭐⭐⭐⭐ 直接优化 SQL |
| 批量操作 | ⭐⭐ 效率低 | ⭐⭐⭐⭐⭐ 批量插入/更新 |
